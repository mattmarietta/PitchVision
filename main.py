...
def main():
    ...
    #Draw output vid
    ## Draw object object_tracks
    #print(f"Number of video frames: {len(video_frames)}")
    #print(f"Number of tracked frames: {len(object_object_tracks['players'])}")

# Get object positions 
    tracker.add_position_to_tracks(object_tracks)
    ...
    camera_movement_estimator.add_adjust_positions_to_tracks(object_tracks,camera_movement_per_frame)


    # View Trasnformer
    view_transformer = ViewTransformer()
    view_transformer.add_transformed_position_to_tracks(object_tracks)
    ...
    speed_and_distance_estimator.add_speed_and_distance_to_tracks(object_tracks)

    # Assign Player Teams
    team_assigner = TeamAssigner()
    team_assigner.assign_team_color(video_frames[0], 
                                    object_tracks['players'][0])
    ...
    #Draw output 
    #Draw object Tracks
    output_video_frames = tracker.draw_annotations(video_frames, object_tracks,team_ball_control)

    #Draw Camera movement
    output_video_frames = camera_movement_estimator.draw_camera_movement(output_video_frames,camera_movement_per_frame)

    #Draw Speed and Distance
    speed_and_distance_estimator.draw_speed_and_distance(output_video_frames,object_tracks)
